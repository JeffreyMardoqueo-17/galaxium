...
export function CreateProductModal({ categories, onProductCreate }: CreateProductModalProps) {
  ...
  const [errors, setErrors] = React.useState<Partial<Record<keyof ProductCreateRequest, string>>>({});

  function validateForm() {
    const newErrors: Partial<Record<keyof ProductCreateRequest, string>> = {};

    if (!formData.categoryId || formData.categoryId === 0) newErrors.categoryId = "La categoría es requerida";
    if (!formData.name.trim()) newErrors.name = "El nombre es requerido";
    if (formData.costPrice <= 0) newErrors.costPrice = "El precio de costo debe ser mayor a 0";
    if (formData.salePrice <= 0) newErrors.salePrice = "El precio de venta debe ser mayor a 0";
    if (formData.initialStock < 0) newErrors.initialStock = "El stock inicial no puede ser negativo";
  ...
  function handleInputChange(field: keyof ProductCreateRequest, value: string | number | boolean) {
  ...
  return (
    <DialogPrimitive.Root open={open} onOpenChange={setOpen}>
      ...
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Content className="fixed top-1/2 left-1/2 max-w-lg w-full -translate-x-1/2 -translate-y-1/2 rounded-md bg-white p-6 shadow-lg focus:outline-none">
          <DialogPrimitive.Title className="text-lg font-semibold mb-2">Crear Nuevo Producto</DialogPrimitive.Title>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-center justify-between rounded border p-4">
              <div>
                ...
                <p className="text-sm text-gray-600">El producto estará disponible para la venta</p>
              </div>
