...
export function TopProductsTable({ topProducts, currencyFormatter, numberFormatter }: Props) {
  const maxRevenue = topProducts.products.reduce((acc, current) => {
    const revenue = typeof current.totalRevenue === "number"
      ...
      <CardHeader className="space-y-2">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <CardDescription>Rendimiento por ingresos y unidades vendidas</CardDescription>
          </div>
              ...
              topProducts.products.map((p, idx) => {
                ...
                return (
                  <TableRow key={p.productId} className="hover:bg-muted/25">
                    ...
                    <TableCell className="text-right">{numberFormatter.format(p.totalSold)}</TableCell>
                    <TableCell className="text-right">{currencyFormatter.format(revenue)}</TableCell>
                  </TableRow>
                );
              })
            )}
